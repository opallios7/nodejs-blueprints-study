...
var upload = multer({ dest:'./public/uploads/', limits: {fileSize: 10000000, files:1} });   // 10M 크기로 제한
...
// 실무환경의 에러 핸들러는 StackTrace를 보여주지 않는다.
app.use(function(err, req, res, next) {
    res.status(err.status || 500);
    res.render('error', {
        message: err.message,
